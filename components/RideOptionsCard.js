...
function RideOptionsCard() {
    ...
    const SURBE_CHARGE_RATE = 1.5;

    return (
        <SafeAreaView style={tw`bg-white flex-grow -mt-10`}>
            <View style={tw`relative`}>

                <TouchableOpacity
                    style={tw`absolute  left-5 rounded-full  z-10`}
                ...
                renderItem={({ item: { id, image, title, multiplier }, item }) => (
                    ...
                    >
                        ...
                        <Text style={tw`text-xl`}>
                            {new Intl.NumberFormat('eng-gb', {
                                style: 'currency',
                                currency: 'GBP'
                            }).format(
                                (travelTimeInformation?.duration.value * SURBE_CHARGE_RATE * multiplier) / 100
                            )}
                        </Text>
                    </TouchableOpacity>
                )} />
